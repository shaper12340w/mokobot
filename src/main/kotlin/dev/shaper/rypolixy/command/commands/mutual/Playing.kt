...
class Playing(private val client: Client): MutualCommand {

    override val name           : String                    = "playing"
    override val description    : String                    = "get information of playing track"
    ...
    override suspend fun execute(context: ContextType, res: TextCommand.ResponseData?){
        ...
        if (session == null) {
            ...
            context.sendRespond(
                ResponseType.NORMAL,
                EmbedFrame.musicInfo(session.currentTrack()!!,"")
            )
        }
    }
}